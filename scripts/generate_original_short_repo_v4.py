#!/usr/bin/env python3
"""
VTuber Original Short Generator — Repo Save + YouTube Upload  (v4)
===================================================================
Enhancements over v3:

  1. **YouTube upload** – after saving the video to the repository the script
     uploads it to YouTube via the Data API v3.  Authentication is handled by
     ``youtube_upload.py`` which supports three modes (env-var refresh-token
     for CI, ``token.pickle``, and interactive OAuth).
  2. **Graceful degradation** – if YouTube credentials are missing the upload
     step is skipped and the video is still saved to the repository (same
     behaviour as v3).

Everything else is inherited from v3:
...
_CACHE_META    = CACHE_DIR / "meta_repo_v4.json"
_CACHE_CONTENT = CACHE_DIR / "content_repo_v4.json"
_CACHE_LIVE2D  = CACHE_DIR / "miku_live2d_repo_v4.mp4"
_CACHE_AUDIO   = CACHE_DIR / "speech_repo_v4.mp3"
_CACHE_MUSIC   = CACHE_DIR / "music_repo_v4.mp3"
...
def write_log_entry(
    ...
    if youtube_video_id:
        yt_cell = f"[https://youtu.be/{youtube_video_id}](https://youtu.be/{youtube_video_id})"
        status_cell = "✅ Saved to repository + uploaded to YouTube (v4)"
    else:
        yt_cell = "N/A (upload skipped or failed)"
        status_cell = "✅ Saved to repository (v4, YouTube upload skipped)"

    date_display = timestamp.replace("_", " ").replace("-", ":", 2)

    entry = (
        f"\n## {date_display} UTC [original-vtuber-repo-v4]\n\n"
        f"| Field | Value |\n"
        f"|---|---|\n"
...
# ---------------------------------------------------------------------------
# YouTube upload (graceful — skips if credentials are missing)
# ---------------------------------------------------------------------------
...
def try_upload_to_youtube(
    video_path: Path,
    content: dict,
) -> Optional[str]:
    """Attempt to upload the video to YouTube.

    Returns the YouTube video ID on success, or ``None`` if credentials are
    ...
    try:
        print("[YouTube] Uploading video to YouTube …")
        ...
        print("[YouTube] Video is still saved to the repository.")
        return None


# ---------------------------------------------------------------------------
# Main — with parallel TTS + Live2D capture + YouTube upload
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="vtuber_original_repo_v4_") as tmpdir:
        tmp         = Path(tmpdir)
        ...
        # Upload to YouTube (graceful — skips if creds are missing)
        youtube_video_id = try_upload_to_youtube(video_path, content)

    # Write log entry after temp dir is cleaned up
    write_log_entry(timestamp, content, repo_video_path, youtube_video_id)

    # Update metadata JSON with YouTube info
    if youtube_video_id and repo_video_path:
        meta_path = repo_video_path.with_suffix(".json")
        if meta_path.exists():
            try:
                meta = json.loads(meta_path.read_text(encoding="utf-8"))
                meta["youtube_id"] = youtube_video_id
                meta["youtube_url"] = f"https://youtu.be/{youtube_video_id}"
                meta_path.write_text(
                    json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8"
                )
                print(f"[Save] Updated metadata with YouTube ID: {youtube_video_id}")
            except (json.JSONDecodeError, OSError) as exc:
                print(f"[Save] Warning: could not update metadata: {exc}")
